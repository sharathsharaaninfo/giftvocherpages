...
import { getSendOtpUrl, getValidateOtpUrl } from '../Utils/apiUrls';
...
function OtpModal({  isOpen, onClose, onVerify, onResend, mobile, setOtpModalOpen,setIsOtpVerified}) {
  ...
  const handleVerify = async (otp) => {
    ...
    try {
      ...
      const verificationUrl = getValidateOtpUrl(mobile, otp); // Api URL with mobile number and OTP
  ...
  const handleResend = async () => {
    try {
      const apiUrl = getSendOtpUrl(mobile); // Api URL with mobile number
...
const claimCoupon = async () => {
  ...
  // const handleVerify = async (otp) => {
  //   try {
  //   const verificationUrl =getvalidateotp/${mobile}/${otp}`; // ApI 
  //   console.log(verificationUrl);
  //     const response = await fetch(verificationUrl, {
  //       method: 'GET',
  //       headers: {
